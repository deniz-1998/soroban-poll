...
export default function Dashboard() {
  ...
  return (
    <div className="w-full bg-[#0B0F19] text-gray-100 font-sans p-4">
      {/* Header */}
      <header className="flex justify-between items-center pb-4 border-b border-[#1F2937] mb-6">
        <div className="flex items-center space-x-2 min-w-0">
          <div className="w-3 h-3 bg-indigo-500 rounded-full animate-pulse flex-shrink-0"></div>
          <span className="text-base md:text-lg font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 truncate">
            SoroSim AI Workspace
          </span>
        </div>
        <button
          onClick={connectWallet}
          className="bg-[#1E293B] hover:bg-[#334155] border border-[#334155] text-xs px-3 py-2 rounded-xl font-medium transition-all duration-200 flex-shrink-0"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Simulation Panel */}
        <div className="lg:col-span-5 bg-[#111827]/80 border border-[#1F2937] rounded-2xl p-4 flex flex-col justify-between min-w-0">
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-gray-200">Simulation Controls</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="CC..."
                  className="w-full bg-[#1F2937] border border-[#374151] rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 text-white"
                />
              </div>
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl py-3 font-semibold text-xs transition-all duration-200 shadow-lg shadow-indigo-950/50 mt-6 disabled:opacity-50"
          >
        ...
        <div className="lg:col-span-7 flex flex-col space-y-4 min-w-0">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-[#111827]/80 border border-[#1F2937] rounded-xl p-3 min-w-0 flex flex-col justify-between">
              <span className="block text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1 truncate">Security Audit</span>
              <span className={`text-xs md:text-sm font-bold truncate ${auditStatus === 'SECURE' ? 'text-emerald-400' : auditStatus === 'ANALYZING' ? 'text-blue-400 animate-pulse' : 'text-amber-500'}`}>
                {auditStatus}
              </span>
            </div>
            ...
            <div className="bg-[#111827]/80 border border-[#1F2937] rounded-xl p-3 min-w-0 flex flex-col justify-between">
              ...
              <span className="text-xs md:text-sm font-bold text-gray-200 truncate">{gasEstimated}</span>
            </div>

            <div className="bg-[#111827]/80 border border-[#1F2937] rounded-xl p-3 min-w-0 flex flex-col justify-between">
              <span className="block text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1 truncate">Balance</span>
              <span className="text-xs md:text-sm font-bold text-indigo-400 truncate">{balance} XLM</span>
            </div>
          </div>

          {/* Terminal Logs Window */}
          <div className="flex-grow bg-[#090D16] border border-[#1F2937] rounded-xl p-4 font-mono text-xs flex flex-col h-[260px] min-w-0">
            ...
            <pre className="flex-grow overflow-y-auto text-emerald-400 whitespace-pre-wrap leading-relaxed text-[11px] select-none">
              {logs}
            </pre>
          </div>
